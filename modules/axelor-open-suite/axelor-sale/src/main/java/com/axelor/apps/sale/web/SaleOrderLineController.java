...
public class SaleOrderLineController {
  ...
  public void aiPicLookup(ActionRequest request, ActionResponse response) {
    ...
    MetaFile uploadedImgMFile = saleOrderLine.getAiPictureLookup();

    if (uploadedImgMFile == null) {
      response.setError("No image added, please, upload an image to search", "Null image");
      return;
    }

    String apiUrl = ""; // CHANGE TO ACTUAL URL

    Long uploadedImgMFileId = uploadedImgMFile.getId();
    RequestConfig requestConfig = RequestConfig.custom().build();
    List<NameValuePair> postParameters = new ArrayList<>();
    postParameters.add(
        new BasicNameValuePair("txtSearchedImageId", Long.toString(uploadedImgMFileId)));
    ...
    try {
      ...
      apiResponseEntity = apiResponse.getEntity();
      apiResponseString = EntityUtils.toString(apiResponseEntity);
      apiResponseObject = new JSONObject(apiResponseString);
      JSONObject responseDataObject = apiResponseObject.getJSONObject("data");
      if (!responseDataObject.isNull("unit")) {
        JSONObject unitObject = responseDataObject.getJSONObject("unit");
        Long unitId = unitObject.getLong("id");
        Unit unit = Beans.get(UnitRepository.class).find(unitId);
        response.setAttr("unit", "value:set", unit);
      }
      String code = responseDataObject.getString("code");
      String name = responseDataObject.getString("name");
      Long productId = responseDataObject.getLong("id");
      Product foundProduct = Beans.get(ProductRepository.class).find(productId);
      response.setAttr("product", "value:set", foundProduct);
      response.setValue("productName", "[" + code + "] " + name);
      BigDecimal salePrice =
          BigDecimal.valueOf(Double.valueOf(responseDataObject.getString("salePrice")));
      ...
      try {
        apiResponseString = EntityUtils.toString(apiResponseEntity);
        apiResponseObject = new JSONObject(apiResponseString);
        String apiError = apiResponseObject.getString("error");
        response.setError(apiError, "API Error");
